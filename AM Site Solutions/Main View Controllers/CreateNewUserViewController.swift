...
//        title = "Create New User"
        title = isEditingUser ? TranslationManager.shared.getTranslation(for: "manageUsersScreen.editUserScreenTitle") : TranslationManager.shared.getTranslation(for: "newUserScreen.newUserScreenTitle")
    ...
    @objc func reloadTranslations() {
        // Update the title
        title = isEditingUser ?
            TranslationManager.shared.getTranslation(for: "manageUsersScreen.editUserScreenTitle") :
            TranslationManager.shared.getTranslation(for: "newUserScreen.newUserScreenTitle")
        
        // Update labels
        firstNameLabel.text = TranslationManager.shared.getTranslation(for: "newUserScreen.firstNameLabel")
        surnameLabel.text = TranslationManager.shared.getTranslation(for: "newUserScreen.surnameLabel")
        phoneNumberLabel.text = TranslationManager.shared.getTranslation(for: "newUserScreen.phoneNoLabel")
        userTypeLabel.text = TranslationManager.shared.getTranslation(for: "newUserScreen.userTypeLabel")
        
        // Update placeholders
        firstNameTextField.placeholder = TranslationManager.shared.getTranslation(for: "newUserScreen.firstNamePlaceholder")
        surnameTextField.placeholder = TranslationManager.shared.getTranslation(for: "newUserScreen.surnamePlaceholder")
        phoneNumberTextField.placeholder = TranslationManager.shared.getTranslation(for: "newUserScreen.phoneNoPlaceholder")
        userTypeTextField.placeholder = TranslationManager.shared.getTranslation(for: "newUserScreen.userTypePlaceholder")
...
//        submitButton.setTitle("Submit", for: .normal)
    ...
    @objc func submitButtonTapped() {
        ...
        guard let firstName = firstNameTextField.text, !firstName.isEmpty else {
            showAlert(message: TranslationManager.shared.getTranslation(for: "newUserScreen.pleaseEnterFirstName"))
            return
        }
        
        guard let surname = surnameTextField.text, !surname.isEmpty else {
            showAlert(message: TranslationManager.shared.getTranslation(for: "newUserScreen.pleaseEnterSurname"))
            return
        }
        ...
        if userType == "Please Select" {
            showAlert(message: TranslationManager.shared.getTranslation(for: "newUserScreen.pleaseSelectUserType"))
            return
        }
    ...
    func updateUser(userID: String, firstName: String, surname: String, phoneNumber: String, userType: String) {
        ...
        functions.httpsCallable("updateUser").call([
            ...
            if let error = error {
                print("Error: \(error.localizedDescription)")
                self.showAlert(message: TranslationManager.shared.getTranslation(for: "common.errorTryAgain"))
                return
            }

            if let response = result?.data as? [String: Any], response["success"] as? Bool == true {
                self.showAlertWithDismiss(message: TranslationManager.shared.getTranslation(for: "newUserScreen.userUpdateSuccess"))
            } else {
                self.showAlert(message: TranslationManager.shared.getTranslation(for: "common.unknownError"))
            }
        }
    }
...
//        return phoneNumber.hasPrefix("+353") || phoneNumber.hasPrefix("085") || phoneNumber.hasPrefix("086") || phoneNumber.hasPrefix("087") || phoneNumber.hasPrefix("089") || phoneNumber.hasPrefix("083")
//    }

    func isValidPhoneNumber(_ phoneNumber: String) -> Bool {
        // 1. Check for Irish number formats (using original format)
        let isIrishNumber = phoneNumber.hasPrefix("+353") || phoneNumber.hasPrefix("085") || phoneNumber.hasPrefix("086") || phoneNumber.hasPrefix("087") || phoneNumber.hasPrefix("089") || phoneNumber.hasPrefix("083")

        // 2. Remove non-digit characters (excluding '+') for international check
        var digitsOnly = phoneNumber.components(separatedBy: CharacterSet.decimalDigits.inverted).joined()
        if phoneNumber.hasPrefix("+") {
            digitsOnly = "+" + digitsOnly
        }

        // 3. Check for international format
        let pattern = "^\\+(?:[0-9] ?){6,14}[0-9]$"
        let range = NSRange(location: 0, length: digitsOnly.utf16.count)
        let regex = try! NSRegularExpression(pattern: pattern)
        let isInternationalNumber = regex.firstMatch(in: digitsOnly, options: [], range: range) != nil

        // 4. Return true if either format is valid
        print("Answer is \(isIrishNumber || isInternationalNumber)")
        return isIrishNumber || isInternationalNumber
    }
